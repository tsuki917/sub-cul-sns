...
export default function StarIcon(props: {
  isFavo: boolean;
  setIsFavo: React.Dispatch<React.SetStateAction<boolean>>;
  onClick:()=>void;
  favonum:number

}) {
  const handleClick = () => {
    // setIsFavo 関数を呼び出して、isFavo の値をトグルします
    props.setIsFavo((pre)=>!pre);
    props.onClick()
  };

  return (
    <Button isIconOnly className="bg-white" onClick={handleClick}>
      <p className="px-1">{props.favonum}</p>
      <svg
      ...
      >
          <path
            ...
            fill={`${props.isFavo?"#FFD662":"#FFFFFF"}`}
            stroke="black"
          />
